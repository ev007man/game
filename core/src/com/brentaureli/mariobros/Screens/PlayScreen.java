...
import com.brentaureli.mariobros.Controller;
...
private TextureAtlas spriteSheet;
    ...
    public PlayScreen(MarioBros game){
        atlas = new TextureAtlas("spritesheetfree.txt");

        spriteSheet = new TextureAtlas("spritesheetfree.txt");
    ...
    public void handleInput(float dt){
        ...
        if(player.currentState != Mario.State.DEAD) {
            ...
            if (controller.isRightPressed() && player.b2body.getLinearVelocity().x <= 1.5f)
                player.b2body.applyLinearImpulse(new Vector2(0.1f, 0), player.b2body.getWorldCenter(), true);

            if (controller.isLeftPressed() && player.b2body.getLinearVelocity().x >= -1.5f)
                player.b2body.applyLinearImpulse(new Vector2(-0.1f, 0), player.b2body.getWorldCenter(), true);
            if ( controller.isfired() ) {
                player.fire();
            }
        }
    ...
    public void update(float dt){
        ...
        for(Enemy enemy : creator.getEnemies()) {
            enemy.update(dt, player.getX() / MarioBros.PPM);
